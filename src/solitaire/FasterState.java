...
public class FasterState implements AstarNode {
	...
	private String lines[][] = new String[NUM_COLUMNS][HEIGHT + EXTRA_SPACE];
	...
	*/
	...
	public FasterState(String deck[]) {
		
		String initLines[][] = new String[NUM_COLUMNS][HEIGHT];
		
		for(int i=0; i<initLines.length; i++) {
			for(int j=0; j<HEIGHT; j++) {
				initLines[i][j] = deck[i * HEIGHT + j];
			}
		}
		initState(initLines);
	}
	
	public FasterState(String initLines[][]) {
		initState(initLines);
	...
	private void initState(String initLines[][]) {
		...
		//let the computer know if I'm doing a simple test and not using all suits:
		boolean suitedCardUsed[] = new boolean[NUM_SUITS];

		for(int i=0; i<initLines[0].length; i++) {
			for(int j=0; j<initLines.length; j++) {
				this.lines[i][j] = initLines[j][i].toUpperCase();
				
				if(this.isSuitedCard(this.lines[i][j])) {
					suitedCardUsed[this.getSuitIndex(this.lines[i][j])] = true;
				}
			}
		}
		
		for(int i=0; i<NUM_SUITS; i++) {
			if(suitedCardUsed[i] == false) {
				numDone[i] = NUM_RANKS;
			}
		}
		
		//TODO: if the dragons are described the same way, change their name to X1, X2, X3, X4
		
		for(int i=0; i<initLines[0].length; i++) {
			for(int j=0; j<initLines.length; j++) {
				
				for(int i2 = i; i2<initLines[0].length; i2++) {
					
					int jStart = 0;
					if(i2 == i) {
						jStart = j+1;
					}
					for(int j2 = jStart; j2<initLines.length; j2++) {
						if(this.lines[i][j].equals("") == false 
								&& this.lines[i][j].equals(this.lines[i2][j2])) {
							System.out.println("ERROR: two instances of card: " + this.lines[i][j]);
							System.exit(1);
		...
		System.out.println("After auto moves:");
		doAutoMoves();
	...
	public ArrayList<String> getMoves() {
		...
		if(hasEmptyReserveSlots()) {
			for(int i=0; i<NUM_COLUMNS; i++) {
				if(isColumnEmpty(i) == false) {
					...
					ret.add("move card " + currentCard + " of col " + i + " to reserve slot");
				}
			}
		}
		...
		for(int i=0; i<NUM_COLUMNS; i++) {
			...
			for(int k=0; k<numGroup; k++) {
				...
				//Flag to acknowledge that one empty column is just as good as another.
				boolean alreadyMovedToEmptryColumn = false;
				...
				for(int i2=0; i2<NUM_COLUMNS; i2++) {
					...
					if(this.isColumnEmpty(i2)) {
						...
						if(k < indexBottom) {
							if(alreadyMovedToEmptryColumn == false) {
								ret.add("move card " + currentCard + " of col " + i + " to empty column " + i2);
								alreadyMovedToEmptryColumn = true;
							}
						}

					} else {
						
						String cardBottomMove = lines[i2][this.getIndexBottomCardOfColumn(i2)];
						
						if(cardCouldLetCardMoveUnderIt(cardBottomMove, currentCard)) {
							ret.add("move card " + currentCard + " of col " + i + " to column " + i2);
						}
					}
		...
		for(int r=0; r<this.NUM_RESERVE_SLOTS; r++) {
			if(this.cardsInReserve[r].equals("") == false) {
				
				String currentCard = this.cardsInReserve[r];
				...
				for(int i=0; i<this.NUM_COLUMNS; i++) {
					...
					if(this.isColumnEmpty(i)) {
						...
						if(alreadyMovedToEmptryColumn == false) {
							ret.add("move reserve card " + currentCard + " to empty column " + i);
							alreadyMovedToEmptryColumn  = true;
						}

					} else if(cardCouldLetCardMoveUnderIt(lines[i][this.getIndexBottomCardOfColumn(i)], currentCard)) {
						ret.add("move reserve card " + currentCard + " to non-empty column " + i);
...
ret.add("move all of group " + suitIDString + " to reserve slot");
	...
	public FasterState doMove(String moveDescription) {
		FasterState newState = this.softCopy();
	...
	private FasterState doMoveAndChangeState(String moveDescription) {
		//1st get card to use
...
ret.add("move reserve card " + currentCard + " to done");
	*/
	...
	public long getAdmissibleHeuristic(AstarNode goal) {
		...
		FasterStateMoreColumns initSimplified = new FasterStateMoreColumns(lines, cardsInReserve, numDone);

		ArrayList<AstarNode> path = AstarAlgo.astar(initSimplified, null);
		
		//Path should never be null...
		
		System.out.println("Got heuristic!!! " + "( " + (path.size() - 1) + ")");
		System.out.println(this.toString());
		System.out.println();
		System.out.println();
	...
	/*
	public int getNumGroupsFromCardAToCardB(int colIndex, int indexHigher, int indexLower) {
		...
		String curCard = this.lines[colIndex][indexHigher];
		
		for(int j=indexHigher+1; j<=indexLower; j++) {
			String nextCard = this.lines[colIndex][j];
			
			if(cardCouldLetCardMoveUnderIt(curCard, nextCard)) {
				//next card is part ot the same group
			} else {
				ret++;
			}
		...
		if(ret == 0) {
			System.out.println("ERROR: so far, this should be impossible!");
		}
	...
	*/
	...
	public void moveGroupOfCards(String cardString, int newColumnIndex) {
		...
		if(this.isColumnEmpty(newColumnIndex) == false) {
			startNewColJ = this.getIndexBottomCardOfColumn(newColumnIndex) + 1;
		}
	...
	private String lastMoveDescription = "";
		...
		FasterState newState = new FasterState();
