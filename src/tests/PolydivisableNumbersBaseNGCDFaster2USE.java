...
public class PolydivisableNumbersBaseNGCDFaster2USE {
	...
	public static void main(String[] args) {
		...
		for(int base = 2; base <= 58; base = base +2) {
	...
	public static ArrayList<String> getPolydivisableNumbersBaseN(int base) {
		...
		isCorrectGCDWithBase = new boolean[base-1][base];
	...
	public static ArrayList<String> guessNumber(int base, int indexToAdd, BigInteger currentNumber) {
		...
		/*if(indexToAdd >= maxDepth) {
			System.out.println("Max depth: " + indexToAdd);
			maxDepth = indexToAdd;
		}*/
		
		BigInteger remainder = currentNumber.divideAndRemainder(new BigInteger("" + (indexToAdd + 1)))[1];
		
		BigInteger firstTryBig = new BigInteger("" + (indexToAdd + 1)).subtract(remainder);
		int firstTry = firstTryBig.intValue();
		int jumpAmount = indexToAdd + 1;
		
		//Skip:
		/*if(indexToAdd == 1 && base == 58) {
			if(currentNumber.equals(new BigInteger("" + base))) {
		...
		for(int i=firstTry; i < base; i += jumpAmount) {
			if(used[i] == false && isCorrectGCDWithBase[indexToAdd][i]) {
				...
				/*
				if(base == x &&indexToAdd == 0&& i >=53) {
					System.out.println("Trying to start with " + i);
				} else if(base == x &&indexToAdd == 0) {
					continue;
				}*/
