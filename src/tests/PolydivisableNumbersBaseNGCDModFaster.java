...
public class PolydivisableNumbersBaseNGCDModFaster {
	...
	public static void main(String[] args) {
		...
		for(int base = 2; base < 100; base = base +2) {
	...
	public static BigInteger MOD_FOR_BASE;
	
	public static ArrayList<String> getPolydivisableNumbersBaseN(int base) {
		MOD_FOR_BASE = BigInteger.ONE;
		for(int i=2; i<base; i++) {
			if(MOD_FOR_BASE.divideAndRemainder(new BigInteger("" + i))[1].compareTo(BigInteger.ZERO) != 0) {
				MOD_FOR_BASE = MOD_FOR_BASE.multiply(new BigInteger("" + i).divide(new BigInteger("" + i).gcd(MOD_FOR_BASE)));
			}
		}
		System.out.println(MOD_FOR_BASE);
		
		isCorrectGCDWithBase = new boolean[base][base];
		for(int i=0; i<base; i++) {
			for(int j=1; j<base; j++) {
				if(UtilityFunctions.getGCD(base, i+1) == UtilityFunctions.getGCD(base, j)) {
	...
	public static ArrayList<String> guessNumber(int base, int indexToAdd, BigInteger currentNumber) {
		...
		for(int i=1; i < base; i++) {
			if(used[i] == false && isCorrectGCDWithBase[indexToAdd][i]) {
				...
				if( nextNumber.divideAndRemainder(new BigInteger("" + (indexToAdd + 1)))[1].compareTo(BigInteger.ZERO) == 0) {
					indexToAdd++;
					if(indexToAdd == targetLengthNumber) {
						...
						guessNumber(base, indexToAdd, nextNumber.multiply(new BigInteger(base + "")).divideAndRemainder(MOD_FOR_BASE)[1]);
					}
...
Base 50:
1010001010101000101010100010101010001010101000101
0101010100010101010001010101000101010100010101010
1010001010101000101010100010101010001010101000101
0101010100010101010001010101000101010100010101010
...
0101010100010101010001010101000101010100010101010
1010001010101000101010100010101010001010101000101
0000000001000000000100000000010000000001000000000
1010001010101000101010100010101010001010101000101
0101010100010101010001010101000101010100010101010
1010001010101000101010100010101010001010101000101
0101010100010101010001010101000101010100010101010
