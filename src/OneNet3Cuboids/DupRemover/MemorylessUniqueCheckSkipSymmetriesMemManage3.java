...
public class MemorylessUniqueCheckSkipSymmetriesMemManage3 {
	...
	public MemorylessUniqueCheckSkipSymmetriesMemManage3(CuboidToFoldOn orig) {
		...
		//Top is similar to bottom in Nx1x1 cuboids:
		...
		startingPointsAndRotationsToCheck = PivotCellDescription.getUniqueRotationListsWithCellInfo(orig);

		if(startingPointsAndRotationsToCheck.size() > 1) {
			
			int numRemoved = 0;
			
			for(int i=0; i<startingPointsAndRotationsToCheck.size(); i++) {
				if(startingPointsAndRotationsToCheck.get(i).getCellIndex() == 0
						|| isSimilarCellToStart[startingPointsAndRotationsToCheck.get(i).getCellIndex()]) {
					startingPointsAndRotationsToCheck.remove(i);
					i--;
					numRemoved++;
				}
			}
			...
			if(numRemoved != 1) {
				System.out.println("Oops! Meant to remove 1 starting point, but removed " + numRemoved + ".");
				System.exit(1);
			}
		}
	...
	public boolean isUnique(Coord2D paperToDevelop[], boolean array[][], int origIndexCuboidOnPaper[][]) {
		...
		//New rotation valid check:
		int PAPER_TO_DEVELOP_INDEX_TO_USE = 0;
		
		boolean foundOtherValid = false;
		
		//Utils.printFold(array);
		//Utils.printFoldWithIndex(origIndexCuboidOnPaper);
		...
		for(int i=0; i<startingPointsAndRotationsToCheck.size(); i++) {
			...
			if(isValidSetupAtIndexedStartLocationWithRotation(paperToDevelop, array, PAPER_TO_DEVELOP_INDEX_TO_USE, rotation, index)) {
				
				//TODO: Don't be so lazy and figure out how to change the validSetup array
				// without relying on legacy code!
				// Whatever! The fact that this flag is false most of the time will give a good enough speed up!
				foundOtherValid = true;
			}
		...
		for(int index=1; index<validSetup.length; index++) {
			...
			if(FoldResolveOrderedRegionsSkipSymmetries.getNumUsedNeighbourCellonPaper(
				...
				if(validSetup[index]) {
					...
					if(foundOtherValid == false) {
						System.out.println("Doh! The new rotation valid check said there would not be another valid solution on a different start location.");
						System.out.println("Orig index on paper: " + origIndexCuboidOnPaper[paperToDevelop[index].i][paperToDevelop[index].j]);
						Utils.printFold(array);
						Utils.printFoldWithIndex(origIndexCuboidOnPaper);
