...
public class BasicUniqueCheckImproved {
	...
	public static boolean isUnique(boolean array[][]) {
		...
		long maxDim = Math.max(heightShape, widthShape);
		long minDim = Math.min(heightShape, widthShape);
		...
		if(maxDim != minDim) {
			...
			for(int i=0; i<scores.length; i++) {
				scores[i] = new BigInteger((maxDim * BIG_ENOUGH_NUMBER + minDim) + "");

			}
			
			boolean tooLow[] = new boolean[NUM_REFLECTIONS * NUM_ROTATIONS / CHECK_SYMMETRIES_ONE_DIM_FACTOR];
