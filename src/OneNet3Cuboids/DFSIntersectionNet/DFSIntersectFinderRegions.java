...
public class DFSIntersectFinderRegions {
	...
	public static int Nx1x1BottomCellI = -1;
	public static int Nx1x1BottomCellJ = -1;
	...
	public static void solveCuboidIntersections(CuboidToFoldOn cuboidToBuild, CuboidToFoldOn cuboidToBringAlong, boolean skipSymmetries, SolutionResolverIntersectInterface solutionResolver) {
		...
		Nx1x1BottomCellI = GRID_SIZE/2;
		Nx1x1BottomCellJ = GRID_SIZE/2;
		int START_I = Nx1x1BottomCellI;
		int START_J = Nx1x1BottomCellJ;
		...
		for(int i=0; i<startingPointsAndRotationsToCheck.size(); i++) {
			...
			doDepthFirstSearch(paperToDevelop, indexCuboidOnPaper, paperUsed, cuboid, numCellsUsedDepth, regionsToHandleRevOrder, -1L, skipSymmetries, solutionResolver, cuboidToBringAlongStartRot, indexCuboidOnPaper2ndCuboid, topBottombridgeUsedNx1x1, false, 0);
	...
	public static int tmpDebugFlagDepth = -1;
	...
	public static long doDepthFirstSearch(Coord2D paperToDevelop[], int indexCuboidonPaper[][], boolean paperUsed[][], CuboidToFoldOn cuboid, int numCellsUsedDepth,
			...
			int topBottombridgeUsedNx1x1[], boolean debugFlagNope, int curGlobalMinOrderedCell) {
		...
		if(numCellsUsedDepth == cuboid.getNumCellsToFill()) {
			...
			if(debugFlagNope || tmpDebugFlagNope) {
				...
				System.out.println("debugFlagNopeÈ " + debugFlagNope);
				System.out.println("tmpDebugFlagDepth: " + tmpDebugFlagDepth);
		...
		boolean isSingleMainRegion = false;
		if(regions.length == 1 && regions[0].getCellIndexToOrderOfDev().size() == cuboid.getNumCellsToFill() - numCellsUsedDepth) {
			isSingleMainRegion = true;
		}
		
		//Display debug/what's-going-on update
		numIterations++;
		
		if(numIterations >= 1083569 && numIterations<=1083579 && numCellsUsedDepth == 13) {
			System.out.println("Debug test 1234: " + numIterations);
			System.out.println("Num iterations: " + numIterations);
			System.out.println("debugFlagNopeÈ " + debugFlagNope);
			System.out.println("tmpDebugFlagDepth: " + tmpDebugFlagDepth);
			Utils.printFold(paperUsed);
		...
		for(int i=regions[regionIndex].getMinOrderedCellCouldUsePerRegion(); i<paperToDevelop.length && paperToDevelop[i] != null; i++) {
			...
			for(int j=0; j<neighbours.length; j++) {
				...
				if( !cantAddCellBecauseOfOtherPaperNeighbours) {
					...
					if(isSingleMainRegion) {
						curGlobalMinOrderedCell = i;
					}
						...
						if( ! cuboidToBringAlongStartRot.isCellIndexUsed(indexNeighbourOfNewCell)) {
							...
							if( ! foundABlankNeighbour) {
								...
								if(FoldResolveOrderedRegionsSkipSymmetries.areCellsSepartedCuboid(cuboidToBringAlongStartRot, firstIndex, indexNeighbourOfNewCell)) {
									...
									for(int p=0; p <= curGlobalMinOrderedCell; p++) {
									...
									//TODO: make new root region..

									//TODO: only create once
									int TopBottomBridgeOnlyUsedForNx1x1[] = new int[cuboid.getNumCellsToFill()];
									
									if(cuboidToBringAlongStartRot.getDimensions()[1] == 1 && cuboidToBringAlongStartRot.getDimensions()[2] == 1) {
										//TODO: rebuild fakeTopBottomBridgeUsedNx1x1
										
										TopBottomBridgeOnlyUsedForNx1x1 = rebuildTopBottomBridge(indexCuboidOnPaper2ndCuboid, Nx1x1BottomCellI, Nx1x1BottomCellJ, cuboidToBringAlongStartRot);
										
									}
									
									Region regionTmp[] = new Region[numNewWays + 1];
									...
									for(int k=0; k<numNewWays + 1; k++) {
										...
										if(k==0) {
											regionTmp[k] = new Region(cuboidToBringAlongStartRot, regionForOtherCuboid, firstIndex, 0);
										} else if(k == 1) {
											regionTmp[k] = new Region(cuboidToBringAlongStartRot, regionForOtherCuboid, indexNeighbourOfNewCell, 0);	
										} else if(k == 2) {
											regionTmp[k] = new Region(cuboidToBringAlongStartRot, regionForOtherCuboid, indexNeighbourThirdWay, 0);
										//END TODO: this is wrong...
										...
										if( isDubiousOrSmallRegionAfterSplitOtherCuboid(cuboidToBringAlongStartRot, regionForOtherCuboid, regionTmp[k], indexNewCell)
											&& ! regionHasAtLeastOneSolution(paperToDevelop, indexCuboidOnPaper2ndCuboid,
								                       ...
								                       //Don't skip symmetries! (TODO: skip if going back to main nx1x1)
								                       false,
											...
											/*if(cuboidToBringAlongStartRot.getDimensions()[1] == 1 && cuboidToBringAlongStartRot.getDimensions()[2] == 1) {
												...
												System.out.println("Not Double switch!");
											}*/
											//TODO: rename variable:
											
											if(numCellsUsedDepth + regions[regions.length - 1].getNumCellsInRegion() != Utils.getTotalArea(cuboid.getDimensions())) {
												//System.out.println("TEST " + numIterations + "(depth = );
												
												tmpDebugFlagNope = true;
												tmpDebugFlagDepth = numCellsUsedDepth;
									...
									//TODO: count the number of passes(times the filter let it go through) for debug.
				...
				if( ! cantAddCellBecauseOfOtherPaperNeighbours) {
					...
					retDuplicateSolutions += doDepthFirstSearch(paperToDevelop, indexCuboidonPaper, paperUsed, cuboid, numCellsUsedDepth, regions, newLimitDupSolutions, skipSymmetries, solutionResolver, cuboidToBringAlongStartRot, indexCuboidOnPaper2ndCuboid, topBottombridgeUsedNx1x1, debugFlagNope, curGlobalMinOrderedCell);
	...
	public static boolean depthFirstAlgoWillFindAsolutionInRegionIndex(Coord2D paperToDevelop[], int indexCuboidonPaper[][],
			...
			int topBottombridgeUsedNx1x1[], int curGlobalMinOrderedCell) {
		
		
		Region regionArgToUse[] = new Region[1];
		regionArgToUse[0] = regions[regionIndex];
		
		if(doDepthFirstSearch(paperToDevelop, indexCuboidonPaper, paperUsed, cuboid, numCellsUsedDepth, regionArgToUse, 0L, skipSymmetries, null, cuboidToBringAlongStartRot, indexCuboidOnPaper2ndCuboid, topBottombridgeUsedNx1x1, false, curGlobalMinOrderedCell)
				> 0L) {
			return true;
		} else {
	...
	public static boolean depthFirstAlgoWillFindOnly1solutionInRegionIndex(Coord2D paperToDevelop[], int indexCuboidonPaper[][],
			...
			int curGlobalMinOrderedCell) {
		
		
		Region regionArgToUse[] = new Region[1];
		regionArgToUse[0] = regions[regionIndex];
		
		if(doDepthFirstSearch(paperToDevelop, indexCuboidonPaper, paperUsed, cuboid, numCellsUsedDepth, regionArgToUse, 1L, skipSymmetries, null, cuboidToBringAlongStartRot, indexCuboidOnPaper2ndCuboid, topBottombridgeUsedNx1x1, false, curGlobalMinOrderedCell)
				== 1L) {
			return true;
		} else {
	...
	public static long numBreak3 = 0;
	...
	public static Region[] splitRegionsIfNewCellSplitsRegions(Coord2D paperToDevelop[], int indexCuboidonPaper[][],
					...
					if(FoldResolveOrderedRegionsSkipSymmetries.areCellsSepartedCuboid(cuboid, firstIndex, indexNeighbourOfNewCell)) {
						...
						for(int i2=regions.length - 1 - numNewWays; i2< regions.length; i2++) {
							...
							regionHasOneSolution[i2] = depthFirstAlgoWillFindOnly1solutionInRegionIndex(paperToDevelop, indexCuboidonPaper,
									...
									topBottombridgeUsedNx1x1, curGlobalMinOrderedCell);
	 ...
	 //pre: cell is added in splitRegionsIfNewCellSplitsRegions
	...
	public static void main(String args[]) {
		...
		//Split by other regions almost 4 hours :(
		//Maybe I just need to go allin! and switch back and forth and switch whenever it reduces the depth.
