...
public class Taxicab4OneArrayCheck2018SpaceSavingTODO {

	//NUM_POWER should be 5 to tackle the challenge,
	//but it could also be a prime greater than 2
	public static int NUM_POWER = 5;
	
	//allow for lots of checking:
	public static int NUM_MODS_LISTS = 6;
	//TODO: after 2-3 checks, actually do the math!
	
	//Start with 10 million.
	public static int RADIUS = 2000000;
	
	public static double CHECK_LIST_MULT = 4.0;
	...
	public static void main(String[] args) {
		...
		int MOD_SCALE = RADIUS;
		

		
		CHECK_NEXT_ROOT:
		for(int currentMod = MOD_SCALE; mods[NUM_MODS_LISTS - 1] <= 0 && modsFound < NUM_MODS_LISTS; currentMod++) {
		...
		System.out.println("End finding mods to work with");
		...
		for(int i=0; i<NUM_MODS_LISTS ;i++) {
			arrayForw[i] = new int[mods[i] + 1];
			arrayBack[i] = new int[mods[i] + 1];
		...
		for(int modAnswer = 0; modAnswer<mods[0]; modAnswer++) {
			...
			//found is NOT faster...
			boolean found[] = new boolean[mods[1] + EXTRA_SPACE];
			...
			for(int a1pow = 1; a1pow<=mods[0]; a1pow++) {
				...
				int startA = 0;
				if( a1 < b1 ) {
					//TODO: try to setup a space saving start
					//270
					//234180
				...
				for(int i=startA; i<10; i++) {
					for(int j=0; j<i; j++) {
						//TODO compare like with like (if a1 > 2 *a2, it can't work
						int tmmA = a1 + mods[0]*i;
						int tmpb = b1 + mods[0]*j;
						//....
					}
				}
		...
		System.out.println("Number of answers under 1000000:" + answers);
