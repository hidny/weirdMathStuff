...
public class Taxicab4BigModOLD {
	...
	public static int RADIUS = 1000000;
	
	public static double checkListScale = 1.0;
	
	public static int INITIAL_SPACE_FOR_ELEMENTS = 3;
	//Trying:
	//10000005
	//20000011
	//20000018
	...
	public static void main(String[] args) {
		...
		for(int currentMod = RADIUS; mods[NUM_MODS_LISTS - 1] <= 0 && modsFound < NUM_MODS_LISTS; currentMod++) {
			...
			if(modsFound == 1) {
				currentMod = (int)(checkListScale * currentMod);
			}
		}
		...
		for(int modAnswer = 1*RADIUS/4; modAnswer<mods[0]; modAnswer++) {
			if(mods[0] > 1000) {
			...
			for(int a1pow = 0; a1pow<mods[0]; a1pow++) {
				...
				//Look at the next NUM_MODS-1 lists and check if a^5 + b^5 could match something
				...
				for(int i=0; i<NUM_MODS_LISTS-1; i++) {
					...
					//If one of the lists has nothing, we already know there won't be a match
					if(indexTempList[i][0] == 0) {
						keepChecking = false;
						break;
					} else if(indexTempList[i][0] == 1) {
						if(tempNum == -1) {
							tempNum = indexTempList[i][1];
						} else if(tempNum != indexTempList[i][1]) {
							keepChecking = false;
							break;
						}
					}
				...
				//Look for matches in next NUM_MODS-1 lists
				if(keepChecking == true) {
					...
					for(int i=1;  i<=indexTempList[0][0]; i++) {
						...
						for(int j=1; j<indexTempList.length; j++) {
							...
							//This case has already been covered!
